...
const initSocketHandlers = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Authorities join a dedicated room to receive volunteer locations
    socket.on('join_authorities', () => {
      if (socket.authenticated) {
        socket.join('authorities')
        console.log(`🛡️ Socket ${socket.id} joined authorities room`)
      }
    })
    ...
    socket.on('volunteer_location', async ({ lat, lng }) => {
      ...
      // Only emit to authority/admin rooms, not all clients
      socket.to('authorities').emit('volunteer_moved', {
