...
export default function ReportEmergency() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmittedId(res.data.data?.incidentId || '')
      setSubmitted(true)
    } catch (err) {
      console.error(err)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background: 'radial-gradient(circle at 50% 40%, #0a0f2e 0%, #020817 70%)' }}>
        ...
        >
          ...
          <p className="font-mono text-sm text-cyan-400 mb-8">ID: {submittedId || "INC-????"}</p>
        ...
        <motion.form onSubmit={handleSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          ...
          {step === 3 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              {submitError && (
                <p className="font-body text-sm text-red-400 text-center py-2">{submitError}</p>
              )}
